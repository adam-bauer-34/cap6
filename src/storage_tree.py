...
class BaseStorageTree(object, metaclass=ABCMeta):
    ...
    decision_times : ndarray or list
        array of years from start where decisions about mitigation levels are
        done
    ...
    def last_period(self):
        ...
        if self.decision_times[-1]%5 == 0:
            return self.decision_times[-1]
        else:
            return self.decision_times[-1] - 1
...
class SmallStorageTree(BaseStorageTree):
    ...
    def __init__(self, decision_times):
        ...
        if self.decision_times[0] == 0:
            super(SmallStorageTree, self).__init__(decision_times)
            for i in range(0, len(self.decision_times)):
                periods.append(decision_times[i])
        else:
            offset_decision_times = []
            for i in range(0, len(self.decision_times)):
                periods.append(self.decision_times[i] - 1)
                offset_decision_times.append(decision_times[i] - 1)
            super(SmallStorageTree, self).__init__(offset_decision_times)
...
class BigStorageTree(BaseStorageTree):
    ...
    def __init__(self, subinterval_len, decision_times):
        ...
        if self.decision_times[0] == 0:
            ...
            offset_decision_times = []
            for i in range(0, len(decision_times)):
                offset_decision_times.append(decision_times[i] - 1)
            super(BigStorageTree, self).__init__(offset_decision_times)
            self.periods = np.arange(0, self.decision_times[-1] + self.subinterval_len - 1, self.subinterval_len)
            #print(self.periods)
        self._init_tree()
